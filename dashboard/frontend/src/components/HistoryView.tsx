...
export const HistoryView = () => {
  ...
  const generateOutreach = async (recruiterName: string, roleName: string) => {
    ...
    try {
      ...
      setOutreachModal(prev => ({ ...prev, loading: false, text: 'Erro ao gerar mensagem: ' + String(e) }));
    }
  };
  ...
  const getStatusBadge = (vagaStatus: string, candStatus: string) => {
    // 1. Prioridade tática: Alerta sniper
    if (vagaStatus === 'ALERTA_MANUAL') {
      return <span className="px-3 py-1 bg-rose-500/10 text-rose-600 border border-rose-500/20 rounded-full text-xs font-bold uppercase tracking-wider animate-pulse flex items-center gap-1.5"><span className="w-1.5 h-1.5 bg-rose-500 rounded-full"></span>ALVO MANUAL</span>;
    }
    // 2. Erros ou Descartes
    if (vagaStatus === 'REJEITADO_PRESENCIAL' || vagaStatus === 'DESCARTADA') {
      return <span className="px-3 py-1 bg-gray-100 text-gray-500 border border-gray-200 rounded-full text-xs font-bold uppercase tracking-wider">Descartada</span>;
    }
    if (candStatus === 'ERRO') {
      return <span className="px-3 py-1 bg-orange-100 text-orange-600 border border-orange-200 rounded-full text-xs font-bold uppercase tracking-wider">Falha</span>;
    }
    // 3. Sucessos de Aplicação
    if (candStatus === 'APLICADA' || candStatus === 'ENVIADA' || candStatus === 'CONFIRMADA') {
      return <span className="px-3 py-1 bg-teal-500/10 text-teal-600 border border-teal-500/20 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1.5"><span className="w-1.5 h-1.5 bg-teal-500 rounded-full"></span>Aplicada</span>;
    }
    // 4. Default: Pendente, Forjado (na fila)
    return <span className="px-3 py-1 bg-indigo-50 text-indigo-600 border border-indigo-100 rounded-full text-xs font-bold uppercase tracking-wider">Pendente</span>;
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto p-10 bg-gradient-to-br from-[#f9f9f9] to-gray-100/50">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          <h1 className="text-3xl font-extrabold text-[#1a1c1c] tracking-tight mb-2">Terminal Operacional</h1>
          <p className="text-gray-500 text-sm font-medium">Controle de engajamento automático e alertas de snipers.</p>
        </div>
        <div className="flex gap-2">
          {['ALL', 'MANUAL', 'APLICADAS', 'REJEITADAS'].map(f => (
            ...
            >
              {f === 'ALL' ? 'Todas' : f === 'MANUAL' ? 'Alvos Manuais' : f}
            </button>
          ))}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {filteredData.length === 0 ? (
            <div className="col-span-full py-20 text-center text-gray-400 font-medium">Nenhum evento registrado nesta aba.</div>
          ) : (
            filteredData.map(v => (
              <div key={v.vaga_id} className="relative group bg-white/70 backdrop-blur-md border border-white/40 shadow-sm hover:shadow-lg rounded-2xl p-6 transition-all duration-300 flex flex-col justify-between overflow-hidden">
                ...
                <div>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <a href={v.url} target="_blank" rel="noreferrer" className="text-gray-400 hover:text-indigo-600 transition-colors">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                    </a>
                  </div>

                  <h3 className="font-bold text-[#1a1c1c] text-lg leading-snug mb-1 line-clamp-2">{v.titulo || 'Vaga Desconhecida'}</h3>
                  <p className="text-gray-500 text-sm font-medium mb-4">{v.empresa || 'Empresa Confidencial'}</p>
                </div>
                
                <div className="mt-4 pt-4 border-t border-gray-100 flex justify-between items-center">
                  <div className="text-xs text-gray-400 font-medium">
                    {new Date(v.criado_em).toLocaleDateString('pt-BR', {day: '2-digit', month: 'short', year: 'numeric'})}
                  </div>
                  
                  {/* Se for Manual e Tivermos o Recrutador, exibe botão tático */}
                  {v.vaga_status === 'ALERTA_MANUAL' && (
                    <button 
           ...
           <div className="bg-white rounded-2xl shadow-xl border border-gray-100 w-full max-w-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
              ...
              <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                <h2 className="font-extrabold text-[#1a1c1c] flex items-center gap-2">
                  ...
                  Sintetizador de Abordagem Mágica
                </h2>
              ...
              <div className="p-8">
                 {outreachModal.loading ? (
                    <div className="flex flex-col items-center justify-center space-y-4 py-10">
                      ...
                      <p className="text-gray-500 font-medium text-sm">Forjando quebra de ATS para {outreachModal.recruiter}...</p>
                    </div>
                 ) : (
                    <div className="space-y-6">
                      ...
                      <div className="px-4 py-3 bg-amber-50 text-amber-800 border border-amber-200 rounded-xl text-xs font-medium flex items-start gap-3">
                        ...
                        <p><strong>Aviso Tático:</strong> Você tem 300 caracteres ao convidar alguém pro LinkedIn sem Sales Navigator. Se o texto gerado for muito longo, corte saudações desnecessárias. Vá direto ao ponto técnico que você e o Tech Recruiter dividem dor.</p>
                      </div>
                    </div>
                 )}
              </div>
